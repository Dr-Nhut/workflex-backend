...
class JobController {
    ...
    update(req, res, next) {
        const id = req.params.id;
        const { name = null, description = null, categoryId = null, employerId = null, maxBudget = null, experience = null, duration = null, bidDeadline = null, dateStart = null, type = null, status = null, reasonRefused = null } = req.body;

        const sql = `UPDATE job SET name = IFNULL(?, name), description = IFNULL(?, description), categoryId = IFNULL(?, categoryId), employerId = IFNULL(?, employerId), maxBudget = IFNULL(?, maxBudget), experience = IFNULL(?, experience), duration = IFNULL(?, duration), bidDeadline = IFNULL(?, bidDeadline), dateStart = IFNULL(?, dateStart), type = IFNULL(?, type), status = IFNULL(?, status), reasonRefused = IFNULL(?, reasonRefused) WHERE id='${id}';`
        conn.promise().query(sql, [name, description, categoryId, employerId, maxBudget, experience, duration, bidDeadline, dateStart, type, status, reasonRefused])
