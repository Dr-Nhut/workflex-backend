...
class RecommendationController {
    getFreelancerForNewJob(req, res, next) {
        ...
        conn.promise().query(sql)
            ...
            .then((response) => {
                ...
                req.result = req.result.map((item) => ({
    ...
    getFreelancerForSendEmail(req, res, next) {
        // 1. Get all freelancer of category job
        const categoryId = req.query.categoryId;
        const sql = `SELECT user.id as userId, user.fullname FROM usercategory LEFT JOIN user ON usercategory.userId=user.id WHERE categoryId='${categoryId}' AND user.role='fre'`;
        ...
        conn.promise().query(sql)
            ...
            .then((response) => {
                ...
                req.topFreelancers = req.result.slice(0.10).map((freelancer) => freelancer.freelancerId);
                next()
            })
            .catch(err => console.error(err));
    }
            ...
            .then(([rows, fields]) => {
                ...
                newSimilarJobs.forEach((job, index) => {
                    ...
                    if (!isAppear) {
                        result.push({
                            job,
                            number: newSimilarJobs.filter(item => item === job).length
                        })
                    }
                })
